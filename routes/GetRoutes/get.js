...
router.get('parent/:company/:factory?/:godown?',async (req,res)=>{
    const result = {}
...
router.get('child/:value',async (req,res)=>{
    const result = {}
    if (companies.find({name:req.params.value})){
        res.json({total_sales:companies.find({name:req.params.value}).find({total_sales:1})})
    }
    else if (factories.find({name:req.params.value})){
        res.json({total_sales:factories.find({name:req.params.value}).find({total_sales:1})})
    }
    else if (godowns.find({name:req.params.value})){
        res.json({total_sales:godowns.find({name:req.params.value}).find({total_sales:1})})
    }
